...
access_token=$(jq -r '.access_token' access_response.json)
token_type=$(jq -r '.token_type' access_response.json)
...
for artist_id in $spotify_ids; do
    curl -X GET "https://api.spotify.com/v1/artists/$artist_id/albums" -H "Authorization: $token_type $access_token" -o "data/albums/${artist_id}_albums.json"
done
